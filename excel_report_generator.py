    ...
    def to_excel_row(self) -> dict:
        ...
        try:
            return {
                ...
                "Уверенность": f"{self.confidence:.2f}" if self.confidence is not None else ""
            }
        except Exception as e:
            logger.warning(f"Ошибка при создании строки Excel: {str(e)}")
            # Return a row with minimal data to prevent complete failure
            return {
                "Тип документа": self.type,
                "Номер": self.number,
                "Дата": "",
...
class ExcelReportGenerator:
    ...
    def create_report(self, report_data: Dict[str, Any], output_path: str) -> str:
        """
    ...
    def _create_legal_api_summary(self, documents: List[Document]) -> List[Dict[str, Any]]:
        """Создание сводки по результатам Legal API / Create Legal API summary"""
        summary_data = []
        for doc in documents:
            if doc.legal_api_status:
                summary_data.append({
                    ...
                    'Номер': doc.number,
                    'Название': doc.title,
                    'Статус Legal API': doc.legal_api_status,
                    'URL': doc.validation_url or ''
                })
        return summary_data
    ...
    def _format_worksheet(self, worksheet, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            # Выравнивание по центру для всех ячеек / Center alignment for all cells
